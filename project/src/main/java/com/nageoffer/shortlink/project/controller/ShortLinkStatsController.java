...
public class ShortLinkStatsController {
    ...
    public Result<ShortLinkStatsRespDTO> ShortLinkStats(ShortLinkStatsReqDTO reqDTO) {
        return Results.success(shortLinkStatsService.oneShortLinkStats(reqDTO));
    }
    /**
     * 获取单个短链指定时间范围内的访问记录
     */
    @GetMapping("/api/short-link/v1/stats/access-record")
    public Result<IPage<ShortLinkStatsAccessRecordRespDTO>> getShortLinkStatsAccessRecord(ShortLinkStatsAccessRecordReqDTO reqDTO) {
        return Results.success(shortLinkStatsService.getShortLinkStatsAccessRecord(reqDTO));
    }
    /**
     * 访问分组短链接指定时间内监控数据
     */
